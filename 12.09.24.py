def to_signed_binary(num, bits=32):
...
def from_signed_binary(binary_str):
    """Преобразует бинарную строку с дополнительным кодом обратно в число"""
    bits = len(binary_str)
...
def to_hex(num, bits=32):
    """Преобразует число в шестнадцатеричное представление с заданным количеством бит"""
    if num >= 0:
...
def from_hex(hex_str, bits=32):
    """Преобразует шестнадцатеричную строку обратно в число с заданным количеством бит"""
    try:
        num = int(hex_str, 16)
        if num >= (1 << (bits - 1)): 
            num -= (1 << bits)
